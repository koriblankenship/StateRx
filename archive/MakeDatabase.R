...
# remove lat/lon = 0 and NA, remove yard waste
# binder2 <- binder %>%
#   filter(LAT_PERMIT > 0) %>%
#   drop_na(LAT_PERMIT) %>%
#   drop_na(LON_PERMIT) %>%
...
# # (permits/time) number unique burns/year
...
#sum2 <- sum(p_permits_time$count)

ggplot(data = p_permits_time) +
  aes(x = YEAR, y = count, fill = BURNTYPE_CLASSIFIED) + 
  geom_bar(position='stack', stat='identity') +
  facet_grid(cols = vars(STATE)) 
  # scale_fill_manual(values = mycolors) + #cause colors
  # theme(panel.background = element_blank(), #remove grey background
  #       text = element_text(size = 11, family = "A")) + #font size and style
  # scale_x_continuous(breaks = seq(1995, 2020, 10), name = "Year") + #x breaks and label
  # scale_y_continuous("Number of ignitions/10,000 km2") + #y label
  # labs(fill = "Cause")  #legend label
 
#  box plot of permited acres for broadcast over time
# WA, NV?
p_permit_broadcast <- binder_unique2 %>%
  group_by(STATE, YEAR) %>%
  filter(BURNTYPE_CLASSIFIED == "Broadcast") %>%
  summarise(acres_permit = sum(SUM_PERMIT))

ggplot(data = p_permit_broadcast) +
  aes(x = YEAR, y = acres_permit) + 
  geom_bar(position='stack', stat='identity') +
  facet_grid(cols = vars(STATE)) 

p_request_broadcast <- binder_unique2 %>%
  group_by(STATE, YEAR) %>%
  filter(BURNTYPE_CLASSIFIED == "Broadcast") %>%          
  summarise(acres_max_request = sum(MAX_REQUEST))
p_broadcast_time <- left_join(p_permit_broadcast, p_request_broadcast)

#  completed acres for broadcast over time
# WA?
p_complete_broadcast <- binder_unique2 %>%
  group_by(STATE, YEAR) %>%
  filter(BURNTYPE_CLASSIFIED == "Broadcast") %>%
  summarise(acres_complete = sum(SUM_COMPLETE)) %>%
  filter(acres_complete > 0) ###????

ggplot(p_complete_broadcast, aes(x = STATE, y = acres_complete)) +
  geom_boxplot() 


# fire size histogram for completed burns 
p_complete_firesize <- binder_unique2 %>%
  filter(BURNTYPE_CLASSIFIED == "Broadcast") %>%
...
# average fire size - bar plot
p_avg_firesize <- p_complete_firesize %>%
  group_by(STATE) %>%
  summarise(avg_firesize = mean(SUM_COMPLETE))
ggplot(data = p_avg_firesize) +
  aes(x = STATE, y = avg_firesize) + 
  geom_bar(position='stack', stat='identity')
